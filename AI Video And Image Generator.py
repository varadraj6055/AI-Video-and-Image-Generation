...
def generate_image(prompt, device, iteration, user_id, seed=None):
    ...
    user_directory = Path(f"generated_content/{user_id}")
    user_directory.mkdir(parents=True, exist_ok=True)

    filename = user_directory / f"{sanitized_prompt}_generated_image_{iteration}.png"
    image.save(filename)
    return filename

def create_video(filenames, prompt, user_id, fps=1):
    sanitized_prompt = re.sub(r'\W+', '_', prompt)
    user_directory = Path(f"generated_content/{user_id}")
    output_name = user_directory / f"{sanitized_prompt}_generated_video.mp4"
    ...
    ffmpeg_cmd = [
        "ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", "file_list.txt", "-c:v",
        "libx264", "-profile:v", "high", "-crf", "20", "-pix_fmt", "yuv420p", "-r", str(fps), output_name
    ]

    subprocess.run(ffmpeg_cmd)
    os.remove("file_list.txt")

def notify_user(user_id, content_type):
    print(f"Notification: {content_type} generation for user '{user_id}' is complete. Files are available in 'generated_content/{user_id}/'")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate images and videos from text prompt.")
    parser.add_argument("prompt", type=str, help="Text prompt for image generation.")
    parser.add_argument("--user_id", type=str, required=True, help="Unique user identifier.")
    ...
    for i in range(iterations):
        generated_filename = generate_image(prompt, device, i, user_id, seed)
        generated_filenames.append(generated_filename)

    if iterations > 1:
        create_video(generated_filenames, prompt, user_id, args.fps)

    notify_user(user_id, "Image and Video")
